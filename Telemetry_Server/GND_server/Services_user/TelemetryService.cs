...
namespace GNDServer2.Services_user
{
    ...
    internal class TelemetryService : IDisposable
    {
        ...
        const float h_break = 0.15f;     // J/(s*oC)
        ...
        const float max_drain_rate = 0.05f;  
        ...
        private TelemetryData Parse(byte[] buffer)
        {
            ...
            if (_thermalTimer.IsRunning)
            {
                ...
                dt = Math.Max(0.003f, Math.Min(dt, 0.05f));
                _lastDt = dt;
            }
            else
            {
            ...
            ushort simulatedTempBrake = (ushort)(T_disc + delta_Temp_Brake);
            T_disc = simulatedTempBrake; // Update the disc temperature
            ...
            ushort simulatedTempTire = (ushort)(T_tire + delta_Temp_Tire);
            T_tire = simulatedTempTire;
