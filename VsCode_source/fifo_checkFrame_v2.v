// IP-core FIFO to storage frame and check frame (Header + CRC). Decoded with Variable LENGTH PAYLOAD
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (st)
                ...
                S_RECEIVE_PAYLOAD_CRC: begin
                    if (i_wr_en) begin
                        mem[wptr[AW-1:0]] <= i_wr_data;
                        wptr <= wptr + 1'b1;

                        if (byte_cnt < payload_stop) begin
                            crc_calc <= (crc_calc >> 8) ^ crc32_table(crc_calc[7:0] ^ i_wr_data);
                        end

                        else begin
                            case (byte_cnt - payload_stop)
                                0: crc_rx[7:0]   <= i_wr_data;
                                1: crc_rx[15:8]  <= i_wr_data;
                                2: crc_rx[23:16] <= i_wr_data;
                                3: crc_rx[31:24] <= i_wr_data;
                            endcase
                        end

                        if (byte_cnt == frame_stop) st <= S_CHECK;
